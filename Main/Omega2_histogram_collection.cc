...
int main(int argc, char** argv)
{
	...
	double omega_value_start = 1.4; // starting value for omega_2
...
//strings for the output filename and centrality filename:
	...
	for (int i = 0 ; i < argc ; i++) 
	{
		...
		if (tmp.compare("-START_OMEGA") == 0 && i+1 < argc) 
		{
			double number = atof(argv[i+1]);
			if (number > 0) 
			{
				omega_value_start = number;
			}
		}
	...
	cout << "events chosen = " << n_ev << "; NITER = " << NITER << "; NITER_CENT = " << NITER_CENT << "; START_OMEGA = " << omega_value_start << "; CENT_CUT = " << cent_cut << "; cent_cut_choice = " << cent_cut_choice << endl;
	
	double omega_value[NITER];
	
	for(int iter = 0; iter < NITER; iter++)
	{
		omega_value[iter] = omega_value_start + 0.1*iter;
	}
	
	TH1D *hm0[NITER_CENT][NITER], *hm0_before[NITER_CENT];
	...
	{
		cout << "omega2_cutvalues[i] = " <<  omega_value[i] << endl;
	}
	...
	Int_t Centrality_tpc, n_tracks_mpd;
	Float_t b0;
...
//create output file:
	...
	for(int iter_cent = 0; iter_cent < NITER_CENT; iter_cent++)
	{
		...
		for(int iter = 0; iter < NITER; iter++)
		{
			sprintf(int_hm0,"hm0_%d_%d",iter_cent,iter);
				
			hm0[iter_cent][iter] = new TH1D(int_hm0,int_hm0,100, 1.07, 1.17);
			hm0[iter_cent][iter]->SetTitle("Lmass after selection");
		...
		for(int iter_cent = 0; iter_cent < NITER_CENT; iter_cent++)
		{
			...
			{	
				for(UInt_t i = 0; i < l0->size(); ++i) //cycle for reco l0
				{
					...
					for(int iter = 0; iter < NITER; iter++)
					{	
						if(lamb->omega2 > omega_value[iter]) hm0[iter_cent][iter]->Fill(lamb->massh);
					}		

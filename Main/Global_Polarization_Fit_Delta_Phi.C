...
void Global_Polarization_Fit_Delta_Phi(TString inFile = "invmass_output.root", const int NITER_CENT = 4)
{
	...
	char *int_angle_diff_inv_mass_clone = new char[NITER_CENT];
	...
	char *int_fitting_fnc_angle_diff_reco = new char[NITER_CENT];
	char *int_fitting_fnc_angle_diff_MC = new char[NITER_CENT];
	char *int_fitting_fnc_angle_diff_reco_2 = new char[NITER_CENT];
...
/*	double star_pol_par[] = {1.580}; // corrected value for 11.5GeV
	...
	TLegend *legend2_1=new TLegend(0.15,0.7,0.35,0.9);
...
//	legend3_1->AddEntry(Polar_hist_STAR,"STAR, Nature548 (2017) 62","p");	
	...
	TFile out("phsd_final_pol.root","recreate");

	Polar_prim_hist_MC->Write();
	Polar_hist_MC->Write();
	Polar_hist_RECO->Write();
	Polar_hist_STAR->Write();
	out.Close();

#ifndef MPDGLOBALPOLARIZATION_RECO_H
#define MPDGLOBALPOLARIZATION_RECO_H
...
class MpdGlobalPolarization_RECO : public MpdAnalysisTask2 {
   ClassDef(MpdGlobalPolarization_RECO, 1);

public:
   MpdGlobalPolarization_RECO() {}
   MpdGlobalPolarization_RECO(const char *name, const char *outputName, const char *analysis_choice, const char *selection_choice);
   ~MpdGlobalPolarization_RECO() {} // TODO: normal descructor with cleaning off histos
...
private:
   ...
   TH1D *hRecognitionFlag     = nullptr;  
   TH1D *hLambPTH             = nullptr;  
   TH2D *hAngle               = nullptr;  
   TH2D *hProbTrueP           = nullptr;  
   TH2D *hProbP               = nullptr;  
   TH2D *hProbTruePi          = nullptr;  
   TH2D *hProbPi              = nullptr;  
   TH2D *hProbTrueK           = nullptr;  
   ...
   float massh, pth, ph, etah, yh, chi2h, disth, path, c2pv, d2pv, masshL, chi2hL, disthL, pathL, angL;
   float etas[2], mcps[2], ps[2], pts[2], chi2s[2], dcas[2], probs[2], chi2sL[2], dcasL[2],  angle, masshL1, chi2hL1;
   ...
   double *kProb, *piProb, *pProb, *eProb;
   int evNo, origs[2], qs[2], dstNo[2], layMx[2], ntr, nLamb, nLamb_MC;
